...
public class TransferEvaluationServiceImpl {
    ...
    public TransferEvaluationResult evaluateTransfer(Long srcId, Long tgtId) {
        ...
        for (TransferRule rule : rules) {
            ...
            // Overlap must meet minimum
            if (overlap < rule.getMinimumOverlapPercentage()) {
                continue;
            }

            int diff = Math.abs(
                    src.getCreditHours() - tgt.getCreditHours()
            );

            Integer tolerance = rule.getCreditHourTolerance();

            // STRICT tolerance check (this fixes test22)
            if (tolerance != null && diff > tolerance) {
                r.setNotes("Credit difference exceeds tolerance");
                continue;
            }
